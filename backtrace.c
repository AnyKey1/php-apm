...
static void debug_print_backtrace_args(zval *arg_array TSRMLS_DC,
                                       smart_str *trace_str);
...
void append_backtrace(smart_str *trace_str TSRMLS_DC) {
  ...
  while (ptr) {
...
#if PHP_VERSION_ID >= 70000
    if (skip->func && skip->func->type != ZEND_INTERNAL_FUNCTION &&
        ZEND_USER_CODE(skip->func->common.type)) {
      filename = (skip->func->op_array.filename)
                     ? ZSTR_VAL(skip->func->op_array.filename)
                     : NULL;
    ...
    if (function_name) {
      if (ptr->object) {
        if (ptr->function_state.function->common.scope) {
          ...
          int dup = zend_get_object_classname(ptr->object, &class_name,
      ...
      if ((!ptr->opline) || ((ptr->opline->opcode == ZEND_DO_FCALL_BY_NAME) ||
                             (ptr->opline->opcode == ZEND_DO_FCALL))) {
        if (ptr->function_state.arguments) {
          arg_array = debug_backtrace_get_args(
              &ptr->function_state.arguments TSRMLS_CC);
        }
      }
    }
#endif
    else {
      /* i know this is kinda ugly, but i'm trying to avoid extra cycles in the
       * main execution loop */
      ...
      if (!ptr->opline || ptr->opline->opcode != ZEND_INCLUDE_OR_EVAL) {
        ...
        case ZEND_REQUIRE_ONCE:
          ...
          break;
        default:
          /* this can actually happen if you use debug_backtrace() in your
           * error_handler and you're in the top-scope */
          function_name = "unknown";
          build_filename_arg = 0;
          break;
        }
    ...
    if (filename) {
      smart_str_appendl(trace_str, ") called at [",
                        sizeof(") called at [") - 1);
...
static void append_flat_zval_r(zval *expr TSRMLS_DC, smart_str *trace_str,
  ...
  case IS_ARRAY:
    ...
    if (ZEND_HASH_GET_APPLY_COUNT(Z_ARRVAL_P(expr)) > 0) {
      smart_str_appendl(trace_str, " *RECURSION*", sizeof(" *RECURSION*") - 1);
      return;
    }
    ZEND_HASH_INC_APPLY_COUNT(Z_ARRVAL_P(expr));
#else
...
#elif PHP_VERSION_ID >= 70000
    ZEND_HASH_DEC_APPLY_COUNT(Z_ARRVAL_P(expr));
#else
    Z_ARRVAL_P(expr)->nApplyCount--;
#endif
    break;
  case IS_OBJECT: {
    ...
    if (Z_OBJ_HANDLER_P(expr, get_class_name)) {
      Z_OBJ_HANDLER_P(expr, get_class_name)(expr, (const char **)&class_name,
                                            &clen, 0 TSRMLS_CC);
    }
    if (class_name) {
      ...
      smart_str_appendl(trace_str, "Unknown Class Object (",
                        sizeof("Unknown Class Object (") - 1);
    }
    if (class_name) {
      efree(class_name);
    }
    ...
    if (properties) {
      ...
      if (++properties->nApplyCount > 1) {
        smart_str_appendl(trace_str, " *RECURSION*",
                          sizeof(" *RECURSION*") - 1);
...
static void append_flat_hash(HashTable *ht TSRMLS_DC, smart_str *trace_str,
  ...
  ZEND_HASH_FOREACH_END();
#else
    switch (zend_hash_get_current_key_ex(ht, &string_key, &str_len, &num_key, 0,
                                         &iterator)) {
    case HASH_KEY_IS_STRING:
      ...
      if (str_len > 0) {
        ...
        smart_str_appendl(trace_str, "*unknown key*",
    ...
    ZEND_HASH_FILL_PACKED(Z_ARRVAL_P(arg_array)) {
      if (call->func->type == ZEND_USER_FUNCTION) {
        ...
        if (ZEND_CALL_NUM_ARGS(call) > first_extra_arg) {
          ...
          p = ZEND_CALL_VAR_NUM(call, call->func->op_array.last_var +
...
static char *apm_addslashes(char *str, uint length, int *new_length) {
  ...
  new_str =
      (char *)safe_emalloc(2, (length ? length : (length = strlen(str))), 1);
  source = str;
  end = source + length;
  target = new_str;
