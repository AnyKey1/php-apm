...
#define Z_OBJ_APPLY_COUNT_P(ov) (Z_OBJ_P(ov)->gc.u.v.nApplyCount)
#define Z_OBJ_INC_APPLY_COUNT_P(ov) (++Z_OBJ_P(ov)->gc.u.v.nApplyCount)
#define Z_OBJ_DEC_APPLY_COUNT_P(ov) (--Z_OBJ_P(ov)->gc.u.v.nApplyCount)
#endif
#else
...
#define PROCESS_EVENT_ARGS                                                     \
  int type, char *error_filename, uint error_lineno, char *msg,                \
      char *trace TSRMLS_DC
...
#ifdef ZTS
#define APM_GLOBAL(driver, v)                                                  \
  TSRMG(apm_globals_id, zend_apm_globals *, driver##_##v)
#else
#define APM_GLOBAL(driver, v) (apm_globals.driver##_##v)
#endif

#if PHP_VERSION_ID >= 70000
#define apm_error_reporting_new_value                                          \
  (new_value && new_value->val) ? atoi(new_value->val)
#else
...
#define APM_DRIVER_CREATE(name)                                                \
  ...
  PHP_INI_MH(OnUpdateAPM##name##ErrorReporting) {                              \
    APM_GLOBAL(name, error_reporting) =                                        \
        (apm_error_reporting_new_value : APM_E_##name);                        \
    return SUCCESS;                                                            \
  }                                                                            \
  ...
  zend_bool apm_driver_##name##_want_event(int event_type, int error_level,    \
                                           char *msg TSRMLS_DC) {              \
    return APM_GLOBAL(name, enabled) &&                                        \
           ((event_type == APM_EVENT_EXCEPTION &&                              \
             APM_GLOBAL(name, exception_mode) == 2) ||                         \
            (event_type == APM_EVENT_ERROR &&                                  \
             ((APM_GLOBAL(name, exception_mode) == 1) ||                       \
              (strncmp(msg, "Uncaught exception", 18) != 0)) &&                \
             (error_level & APM_GLOBAL(name, error_reporting)))) &&            \
           (!APM_G(currently_silenced) ||                                      \
            APM_GLOBAL(name, process_silenced_events));                        \
  }                                                                            \
  zend_bool apm_driver_##name##_want_stats(TSRMLS_D) {                         \
    return APM_GLOBAL(name, enabled) && (APM_GLOBAL(name, stats_enabled));     \
  }                                                                            \
  apm_driver_entry *apm_driver_##name##_create() {                             \
    ...
    driver_entry->driver.error_reporting =                                     \
        apm_driver_##name##_error_reporting;                                   \
...
#define APM_DEBUG(...)                                                         \
  if (APM_G(debugfile)) {                                                      \
    fprintf(APM_G(debugfile), __VA_ARGS__);                                    \
    fflush(APM_G(debugfile));                                                  \
  }
#define APM_SHUTDOWN_DEBUG                                                     \
  if (APM_G(debugfile)) {                                                      \
    fclose(APM_G(debugfile));                                                  \
    APM_G(debugfile) = NULL;                                                   \
  }
#else
...
zend_bool enabled;
/* Application identifier, helps identifying which application is being
 * monitored */
char *application_id;
...
/* Control which exceptions to collect (0: none exceptions collected, 1: collect
 * uncaught exceptions (default), 2: collect ALL exceptions) */
...
#if PHP_VERSION_ID >= 70000
#define zend_is_auto_global_compat(name)                                       \
  (zend_is_auto_global_str(ZEND_STRL((name))))
#define add_assoc_long_compat(array, key, value)                               \
  add_assoc_long_ex((array), (key), (sizeof(key) - 1), (value));
#else
#define zend_is_auto_global_compat(name)                                       \
  (zend_is_auto_global(ZEND_STRL((name)) TSRMLS_CC))
#define add_assoc_long_compat(array, key, value)                               \
  add_assoc_long_ex((array), (key), (sizeof(key)), (value));
#endif
